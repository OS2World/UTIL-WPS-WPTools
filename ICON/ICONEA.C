...
HDIR      hDir;
   ...
   */
   ...
   if (rc == ERROR_BUFFER_OVERFLOW || rc == ERROR_EAS_DIDNT_FIT)
      ...
      if (!pFeaList)
         {
         DebugBox("ICONTOOL", "IconEA: Not enough memory for FeaList(2)!", FALSE);
         free(pGeaList);
         return FALSE;
         }
   ...
   if (ulDataSize > 20000)
      {
//      DebugBox("ICONTOOL","EA data more than 20000!", FALSE);
      free(pGeaList);
      free(pFeaList);
      return FALSE;
      }
...
//      pFeaList->cbList += sizeof (ULONG) - (pFeaList->cbList % sizeof (ULONG));
   ...
   switch (rc)
      ...
      case ERROR_SHARING_VIOLATION:
         DebugBox("ICONTOOL",
            "Sharing violation while writing EA", TRUE);
         return FALSE;
      default        :
         DebugBox("ICONTOOL","Error while writing EA!", TRUE);
         return FALSE;
      }
   return TRUE;
}
